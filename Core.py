...
class DailyQuoteReminder:
    def __init__(self, sdk):
        ...
        if hasattr(sdk, "CommandHandler"):
            sdk.CommandHandler.AddHandle(self.handle_yunhu_command)
            self.logger.info("成功注册 Yunhu 指令处理器")
        if hasattr(sdk, "BotFollowed"):
            sdk.BotFollowed.AddHandle(self.handle_bot_followed)
            self.logger.info("成功注册 BotFollowed 事件处理器")
        ...
        try:
            ...
            if command_name == "提交语录":
                ...
                if not custom_quotes:
                    ...
                    recent_quotes = custom_quotes[-15:][::-1]
            ...
            await self.sdk.MessageSender.Html(recvId=target_id, recvType=chat_type_for_send, content=reply_html)
    ...
    async def send_welcome_message(self, target_id, target_type, platform="ALL"):
        ...
        try:
            ...
            elif hasattr(self.sdk, "MessageSender"):
